...
function css (brick, options) {
  ...
  if (!brick.parentMixing && !brick.parent && !options.noDefaultCSS) {
    waiting++;
...
function body (brick) {
  return render.body(brick, brick.defaultTemplate());
}

function html (brick, options) {
  if (brick.parent) return brick.body();
  return render.container(brick, brick.body(), options);
}
