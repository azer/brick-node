...
function findAllEntries (brick, result, parentDir) {
  result || (result = {});

  var entry = path.join(brick.dir, brick.entry);

  if (parentDir) entry = fixSymLink(brick, entry, parentDir);

  result[entry] = brick;

  brick.mixings && brick.mixings.forEach(function (m) {
    findAllEntries(m, result, path.dirname(entry));
  });
  ...
  for (key in brick.attachments) {
    findAllEntries(brick.attachments[key][0], result, path.dirname(entry));
  }
...
function fixSymLink (brick, filename, parentDir) {
  if (path.basename(path.dirname(path.dirname(filename))) == 'node_modules') return filename;

  var symlink = path.join(parentDir, 'node_modules', brick.key, path.basename(filename));
