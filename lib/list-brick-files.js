...
function isNotBuildDir () {
  ...
  return function (filename) {
    name = filename.path.slice(0, filename.path.indexOf('/', 2));

    if (name.indexOf("./node_modules") == 0 || name.indexOf("node_modules") == 0) {
      debug("Excluding %s since it just keeps the dependencies", filename.path);
      return false;
    }

    if (ind = blacklist.indexOf(name) != -1) {
